...
begin:
    ...
    //basketball-stadium
    string arr8[4]={"Some-Balls","torn clothes","Broken_seats","Broken basket"};
    ...
    for(int i=0;i<4;i++){
     ...
     props temp9[4];
    //stadium
    string arr9[4]={"balls","cards","T-shirts","Key"};
     for(int i=0;i<4;i++){
    ...
    props  temp11[4];
    //Bathroom1
    string arr11[4]={"some pieces of glass","Toothbrush","Wipes","some garbage"};
    for(int i=0;i<4;i++){
        temp11[i].setname(arr11[i]);
    }
    props  temp12[4];
    //Bathroom2
    string arr12[4]={"some pieces of glass","Toothbrush","Wipes","some garbage"};
    for(int i=0;i<4;i++){
    ...
    room Locker_room ("Locker room" , "A big room for changing clothes for the players of the school which have a big bathroom and some places to change clothes for playing and everyone has his special place to change his clothes and some tools for playing and some chairs",temp,2 ,temp14 , 1);
    ...
    room Bathroom1 ("Bathroom" , "when you enter you can find a toilet and dirty toilet papers full of blood when you try to touch anything in it you will be caught by a woman and now you have to go and there is no way and you have no choice only run ",temp,2 ,temp11 ,4 );
    ...
    room Bathroom2 ("The Big Bathroom" , "An enormous bathroom with many toilets and many showers are separated with thin walls but suddenly all these walls are broken and all the bathroom is suddenly full of insects and they ran awards to",temp,2 ,temp12 ,4 );

    room Basketball_stadium ("Basketball stadium" , "A big place like a theater with many places to set which you can pull them and push them at little seconds and this stadium is full of died bodies and bloody clothes on the black walls , Black walls?? Yes black walls because they are full of mud which come from a place which is unknown",temp,2 ,temp8 ,4 );
    ...
    int nAgents=6;
    Monster* monster1 = new Monster("Kafka(a thin, nervous looking creature)",&Basketball_stadium);
    Monster* monster2 = new Monster("Napoleon(once Emperor of the French)",&Basketball_stadium);
    Monster* monster3 = new Monster("Singed(a Headless man)",&Cellar);
    Player* player1 = new Player("Josh",&Locker_room);
    Player* player2 = new Player("Tracy",&Cellar);
    intel_monster* imonster=new intel_monster("Yasser",&Hallway3);
